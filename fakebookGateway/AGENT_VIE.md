...
Development Fusion archive dùng dải localhost chuẩn: Authentication `1001`, SocialGraph `1002`, Recommendation `1003`, Search `1004`, Notification `1005`, Messaging `1006`, và Payment `1007`. Hãy start các service đó trước khi test operation qua Gateway.
...
- Sở hữu email identity, credentials, sessions, OTP, JWT issuing, refresh token rotation, Premium `validDate` và cookie instruction contract; không sở hữu hoặc lưu name, username, birthdate, gender, location hay profile data khác.
